...
const QUESTIONS = [
  ...
  },
  {
    ...
    type: "choice",
    options: ["Texas", "California", "New York", "Florida", "Other Southern state", "Other state"],
  },
  {
    id: "major",
  ...
  },
  {
    ...
    subtitle: "Pick all that apply — we'll tell you if they're realistic",
    type: "multi",
    options: ["Harvard / MIT / Stanford", "UT Austin / A&M", "UCLA / USC", "NYU / Boston U", "Ivy League schools", "None yet — help me find some"],
  },
  {
    id: "extracurricular",
...
export default function AppPage() {
  ...
  function next() {
    ...
    setAnswers(prev => ({ ...prev, [q.id]: q.type === "multi" ? selected : selected[0] }));
    setCurrent(c => c + 1);
    setSelected([]);
  }
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
        <div style={{ width: "100%", maxWidth: "620px" }}>
              ...
              {q.options.map(opt => {
                ...
                return (
                  <button key={opt} onClick={() => selectOption(opt)} style={{ background: isSelected ? "rgba(200,140,36,0.15)" : "var(--navy-mid)", border: `1px solid ${isSelected ? "var(--gold)" : "var(--border-dark)"}`, borderRadius: "var(--radius-md)", padding: "16px 20px", color: isSelected ? "var(--gold-light)" : "rgba(245,240,232,0.8)", fontSize: "15px", fontFamily: "var(--font-body)", textAlign: "left", cursor: "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ width: "20px", height: "20px", borderRadius: q.type === "multi" ? "4px" : "50%", border: `2px solid ${isSelected ? "var(--gold)" : "rgba(245,240,232,0.2)"}`, background: isSelected ? "var(--gold)" : "transparent", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
            {current > 0 && (
              <button onClick={prev} style={{ background: "transparent", border: "1px solid var(--border-dark)", color: "rgba(245,240,232,0.5)", padding: "14px 24px", borderRadius: "var(--radius-md)", fontSize: "15px", fontFamily: "var(--font-body)", cursor: "pointer" }}>← Back</button>
            )}
            <button onClick={next} disabled={q.type === "email" ? !email : selected.length === 0} style={{ flex: 1, background: (q.type === "email" ? !email : selected.length === 0) ? "rgba(200,140,36,0.3)" : "var(--gold)", color: (q.type === "email" ? !email : selected.length === 0) ? "rgba(10,22,40,0.5)" : "var(--navy)", fontWeight: 600, fontSize: "16px", padding: "16px 32px", borderRadius: "var(--radius-md)", border: "none", fontFamily: "var(--font-body)", cursor: selected.length === 0 && !email ? "not-allowed" : "pointer", transition: "all 0.15s" }}>
